...
# Adapted from K.Curti Mackerel code
create_yr_semester_age_length_key <- function(expandedLandings,ageLengthData,filterYrs,plusAge,referenceAge, printConvergence=F) {
...
#      semian.yr.age.data <- comb.age.semian[[as.character(semian)]][[yr]]
  ...
  message(" create matrix for export")
  ...
  for (yr in yrs)
  {
    ...
    for (semian in semianValues)
    {
      ...
      # semian <- 1
      if ((nrow(ALKey.export[[semian]][[yr]])) > 1) { # only if data
