---
title: "3. Length Expansion"
#output: html_document
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{3. Length Expansion}
...
The final stage in data preparation before use in stock assessment models (after [aggregating the data](articles/2aggregationRules.html)) is to expand the landings weight data by the distribution of lengths.
...
For each unique category (YEAR, QTR, NEGEAR, MARKET_CODE) we have both total landings (metric tons) and a sample of fish lengths for this category. The sampled fish lengths are converted to mean weights using the [weight-length relationship](#gen). The resulting sample fish weights are mean weights (metric tons). The weight distribution of sampled fish is then scaled to the weight of total landings. This scaling assumes that the landed (commercial) fish have the same length distribution as the sampled fish.
