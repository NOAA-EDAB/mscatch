...
* Are there *market categories* (they are not on ordinal scale) with total landings greater than 0 but sample sizes less than *X*? (after combining gears). If there are market categories with 0 length samples the user must decide which category to combine. (Default all assigned to (UN)classified. Often these MARKET_CODEs have small landings. We then compare length distributions (Kolmogorov-Smirnov test) of remaining MARKET_CODES to determine aggregation. Currently this is done aggregating length samples over YEAR and QTR. (This can be relaxed at a later date)
