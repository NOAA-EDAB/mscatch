...
* For any NEGEAR/MARKET_CODE combinations where the number of length samples is less than the user defined value (`nLengthSamples`), length samples are borrowed from another NEGEAR type with the same MARKET_CODE. Samples are borrowed from the NEGEAR with the most landings (since is is assumed they will have the most length samples). If there are insufficient length samples, the next NEGEAR in order of total landings is checked. And so on.
