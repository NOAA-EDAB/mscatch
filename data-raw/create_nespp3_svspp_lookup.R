...
create_nespp3_svspp_lookup <- function(channel){
  ...
  cfdbsLookupTable <- RODBC::sqlQuery(channel,sql,errors=TRUE,as.is=TRUE)
  ...
  svdbsLookupTable <- RODBC::sqlQuery(channel,sql,errors=TRUE,as.is=TRUE)
  ...
  speciesLookupTable <- dplyr::left_join(cfdbsLookupTable,svdbsLookupTable, by="SCIENTIFIC_NAME") %>%
    dplyr::select(SCIENTIFIC_NAME,SPECIES_ITIS,dplyr::everything())
